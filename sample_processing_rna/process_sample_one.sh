...
fastqc_script=${HOME}/git/projects/2013/PanCancerTCGA/rerun2017\
/sample_processing_rna/run_fastqc_one.sh
stats_script=${HOME}/git/projects/2013/PanCancerTCGA/rerun2015\
/sample_processing_rna/collect_quick_align_stats.py
count_script=${HOME}/git/projects/2013/PanCancerTCGA\
/count_expression/count_expression.py
as_script=${HOME}/git/software/spladder_pancan_rerun2018/python/spladder.py
bam2fastq=/${HOME}/git/projects/2013/PanCancerTCGA/rerun2017\
/sample_processing_rna/bam2fastq.py
...
genome=/cluster/work/grlab/projects/TCGA/PanCancer/genome\
/hg19_hs37d5_G19.overhang100_STAR_rr18
genomeFasta=/cluster/work/grlab/projects/TCGA/PanCancer/genome\
/hg19_hs37d5/genome.fa
annotation=/cluster/work/grlab/projects/TCGA/PanCancer/annotation\
/gencode.v19.annotation.hs37d5_chr.gtf
annotation_spladder=/cluster/work/grlab/projects/TCGA/PanCancer/annotation\
/gencode.v19.annotation.hs37d5_chr.coding.spladder_rr18.gtf
...
do
    ...
    ### sort bam files
    "$samtools" sort -n "${workdir}/orig/${filebase}" \
        -O BAM -T "$TMPDIR" --threads 3 \
        > "${workdir}/orig/${filebase%bam}sorted.bam"
    ### translate to fastq
    python "$bam2fastq" "${workdir}/orig/${filebase%bam}sorted.bam" 50
done < <(find "${data_dir}/${tid}.${aid}" -name '*.bam')
...
python "$count_script" -m -B -v -A "${outfile_aln}" \
    -a "$annotation" \
    -o "${outfile_cnt1}" \
    > "${logdir}/${tid}.${aid}.exp_count.log" 2>&1 \
    && touch "${donefile_cnt1}" &
outfile_cnt2="${outfile_aln%bam}ex_cnt.non_alt.tab"
donefile_cnt2="${outfile_aln%bam}ex_cnt.non_alt.done"
python "$count_script" -M -m -B -v -A "${outfile_aln}" \
    -a "$annotation" \
    -o "${outfile_cnt2}" \
    > "${logdir}/${tid}.${aid}.exp_count.non_alt.log" 2>&1 \
    && touch "${donefile_cnt2}" &
...
echo \
    "python ${as_script} \
    -b ${outfile_aln} \
    -o ${workdir}/alt_splice \
    -a ${annotation_spladder} \
    -v y -c 2 -M single -n 50 -P y -p n -T n \
    --sparse_bam y --parallel $threads \
    > ${logdir}/${tid}.${aid}.spladder.log \
    && touch $donefile_as"
python "${as_script}" \
    -b "${outfile_aln}" \
    -o "${workdir}/alt_splice" \
    ...
    -v y -c 2 -M single -n 50 -P y -p n -T n \
    --sparse_bam y --parallel "$threads" \
    > "${logdir}/${tid}.${aid}.spladder.log" 2>&1 \
    && touch "$donefile_as"
if [ ! -f "$donefile_as" ]
then
